...
function loadSlide() {
    ...
    function swift() {
        ...
        $('.lists li').on('touchend', function (e) {
            ...
            if (!$('.lists').hasClass("running")) {
                if (end_x < page_x) {
                    ...
                    var id = $(this).find('.slidemodule').attr("data-id");

                    redirect(id);
                }
            }
        });
        $('.lists').on('mousedown', function (e) {
            e.preventDefault();
            var _x = e.clientX;
            page_x = _x;
            end_x = _x;
        ...
        $('.lists li').on('mouseup', function (e) {
            e.preventDefault();
            if (!$('.lists').hasClass("running")&&page_x!=0) {
                if (end_x < page_x) {
                    var index = $(".point .now").index();
                    var len = $(".point li").length;
                    if (index < len - 1) {
